...
### 5. Bộ Video Kiểm Thử Thực Tế Công Trường (Real Construction Footage) & Quy Trình Kiểm Thử
Hệ thống sử dụng các đoạn video ghi hình thực tế tại công trường (raw CCTV/field footage):
```bash
# Xem danh sách và kiểm tra thông số kỹ thuật các video thực tế có sẵn:
python scripts/download_sample_videos.py --list

# Tải thêm video thực tế từ YouTube hoặc nguồn mở bằng yt-dlp:
